...
export default function WaliKelasEwsPage() {
  ...
  // Load kelas milik wali kelas ini
  useEffect(() => {
    async function load() {
      const { kelas: k } = await getMyWaliKelas();
      setKelas(k);
      setKelasLoaded(true);
      if (k.length > 0) setKelasFilter(k[0].id);
    }
    void load();
  }, []);
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <header className="mb-8 border-b border-slate-200 pb-8 dark:border-slate-800">
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            Monitor kehadiran, nilai, dan pelanggaran siswa kelas Anda.
          </p>
        </header>
        ...
        <div className="mb-6 flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:flex-row sm:items-end">
          <div className="flex-1">
            ...
            <select
              ...
              className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-slate-800 disabled:opacity-50 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100"
            >
              {kelas.map((k) => (
        ...
        {/* Ambang batas (read-only, dari DB) */}
        ...
        <section className="mb-8 grid gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-rose-200/80 bg-rose-50/80 p-4 dark:border-rose-900/50 dark:bg-rose-950/40">
            <p className="text-xs font-medium uppercase text-rose-700 dark:text-rose-400">Perlu perhatian</p>
            ...
            <table className="w-full min-w-[680px] text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-slate-500">
                    {kelas.length === 0 ? "Anda belum ditugaskan ke kelas manapun." : "Belum ada data siswa."}
                  </td></tr>
                ) : tableRows.map((row) => (
                  <tr key={row.id} className={row.kritis ? "bg-rose-50/90 dark:bg-rose-950/35" : "hover:bg-slate-50/80 dark:hover:bg-slate-800/40"}>
                    ...
                    <td className="px-4 py-3">
                      {row.kritis ? (
                        <span className="inline-flex items-center rounded-full bg-rose-600 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-white">KRITIS: Panggil BK</span>
                      ) : (
