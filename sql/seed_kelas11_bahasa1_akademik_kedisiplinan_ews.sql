...
-- untuk semua siswa di rombel XI Bahasa 1 (tingkat 11).
--
...
-- Untuk semua rombel tingkat 11 (bukan hanya Bahasa 1), pakai seed_kelas11_akademik_kedisiplinan_ews.sql.
-- Sesuaikan trim(k.nama) bila nama rombel di DB Anda beda.
-- =============================================================================

begin;

-- --- (Opsional) Hapus data tahun AKTIF untuk siswa XI Bahasa 1, lalu isi ulang -
/*
with ay as (
  select id as ay_id
...
sxi as (
  ...
  where k.tingkat = 11
    and trim(k.nama) = 'XI Bahasa 1'
)
delete from public.academic_records ar
using ay, sxi
where ar.student_id = sxi.id
...
using ay, sxi
where v.student_id = sxi.id
  and v.academic_year_id = ay.ay_id;
*/

-- --- Akademik: mapel tingkat 11, S1 & S2, tahun ajaran aktif -------------------
with ay as (
  select id as ay_id
  from public.academic_years
  where is_active = true
...
select
  ...
  v.sem,
  (select ay_id from ay),
  110 + (abs(hashtext(s.id::text || v.sem::text || 'x11')::bigint) % 45),
  case
...
--   where k.tingkat = 11 and trim(k.nama) = 'XI Bahasa 1'
--   group by k.nama;
